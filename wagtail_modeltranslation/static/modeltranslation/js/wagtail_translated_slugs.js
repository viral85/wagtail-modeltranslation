$(document).ready(function() {
    $.each(langs, function(idx, lang_code){
        ...
        $('#id_title_'+lang_code).on('keyup keydown keypress blur', function() {
            if ($('body').hasClass('create') || (!$('#id_slug_'+lang_code).data('previous-val').length || cleanForSlug($('#id_title_'+lang_code).data('previous-val'), true) === $('#id_slug_'+lang_code).data('previous-val'))) {
                ...
                $('#id_slug_'+lang_code).val(cleanForSlug($('#id_title_'+lang_code).val(), true));
            }
        });
    });
});
